...
float average(float* data, uint8_t len) {
  float sum = 0;
...
void setup() {
  ...
  success &= (myICM.enableDMPSensor(INV_ICM20948_SENSOR_ORIENTATION) == ICM_20948_Stat_Ok);
    if(!success)
  {
    Serial.println("DMP sensor Oreintation enable failed");
  }
  success &= (myICM.enableDMPSensor(INV_ICM20948_SENSOR_RAW_ACCELEROMETER) == ICM_20948_Stat_Ok);
    if(!success)
  {
    Serial.println("DMP sensor Accelerometer enable failed");
  }
  success &= (myICM.setDMPODRrate(DMP_ODR_Reg_Quat9, 0) == ICM_20948_Stat_Ok);
    if(!success)
  {
    Serial.println("DMP ODRate Quat9 set failed");
  }
  success &= (myICM.setDMPODRrate(DMP_ODR_Reg_Accel, 0) == ICM_20948_Stat_Ok);
    if(!success)
  {
    Serial.println("DMP ODRate Accel set failed");
  }
...
void loop() {
  ...
  if ((myICM.status == ICM_20948_Stat_Ok) || (myICM.status == ICM_20948_Stat_FIFOMoreDataAvail)) // Was valid data available?
  {
    if ((data.header & DMP_header_bitmap_Accel) > 0) {
      for(int i = 39; i > 0; i--) { // shift in new value
          tdata[i] = tdata[i-1];        //
      }                             //
      tdata[0] = ((double)data.Raw_Accel.Data.Z) / 1073741824.0;       //
 
      for(int i = 0; i < 38; i++) {     // calculate derivative
          tderiv[i] = tdata[i+1] - tdata[i+2]; //
      }                                 //
      if(abs(tdata[0] - tdata[1]) > average(tderiv, 38) * 40) {
        SSq0Characteristic.setValue(TWO);
        SSq0Characteristic.notify();
        SSq1Characteristic.setValue(TWO);
        SSq1Characteristic.notify();
        SSq2Characteristic.setValue(TWO);
        SSq2Characteristic.notify();
        SSq3Characteristic.setValue(TWO);
